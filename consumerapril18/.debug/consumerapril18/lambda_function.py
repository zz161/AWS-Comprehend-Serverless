...
def create_entity(row):
    """Uses AWS Comprehend to Create Entity on a DataFrame"""
...
def apply_entities(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to Create Entities"""
    df['Entities'] = df[column].apply(create_entity)
    return df
...
def lambda_handler(event, context):
    ...
    # Write result to S3
    write_s3(df=df, bucket="fangsentimentalan", name=names)